...
class ReadyStockController extends Controller
{
    ...
    public function getItemwiseProduct(Request $request)
    {
        ...
        $user_id = Auth::user()->id;
        $selectedItem = (array) $request->input('selectedItem', []);
        $procategories = (array) $request->input('procategoryArray', []);
        $purities = (array) $request->input('purity', []);
        $project_id = $request->input('project_id');

        // ✅ First: Get all items user selected
        $getItem = Product::whereIn('Item', $selectedItem)->pluck('Item')->toArray();

        // ✅ Base query for filter options (only depends on Item + Project)
        $optionBaseQuery = Product::join('product_variants', 'product_variants.product_id', '=', 'products.id')
            ->where('products.Project', $request->project_id)
            ->whereIn('products.Item', $getItem)
            ->where('product_variants.qty', '>', 0);

        // Collect all available Procategories for selected Items
        $allProcategoryIds = $optionBaseQuery->clone()
            ->whereNotNull('products.Procatgory')
            ->where('products.Procatgory', '!=', '')
            ->pluck('products.Procatgory')
            ->unique()
            ->toArray();

        // Collect all available Purities for selected Items
        $allPurityIds = $optionBaseQuery->clone()
            ->whereNotNull('product_variants.Purity')
            ->where('product_variants.Purity', '!=', '')
            ->pluck('product_variants.Purity')
            ->unique()
            ->toArray();

        // ✅ Now build main product query
        ...
        if ($project_id) {
            $itemwiseproduct->where('products.Project', $project_id);
        }
        ...
        $itemwiseproduct = $itemwiseproduct->where('product_variants.qty', '>', 0)->orderBy('products.DesignNo', 'ASC');
        ...
        // ✅ Procategory JSONs (only valid for selected Items)
        $procategoryData = Product::join('product_variants', 'product_variants.product_id', '=', 'products.id')
            ->where('products.Project', $request->project_id)
            ->whereIn('products.Procatgory', $allProcategoryIds)
            ->where('product_variants.qty', '>', 0)
            ->whereNotNull('products.Procatgory')
            ->where('products.Procatgory', '!=', '')
        ...
        // ✅ Default Procategory JSONs (also use full dataset, not paginated)
        ...
        // ✅ Purity JSONs (only valid for selected Items)
        $purityData = Product::join('product_variants', 'product_variants.product_id', '=', 'products.id')
            ->where('products.Project', $request->project_id)
            ->whereIn('product_variants.Purity', $allPurityIds)
            ->where('product_variants.qty', '>', 0)
            ->whereNotNull('product_variants.Purity')
            ->where('product_variants.Purity', '!=', '')
        ...
        // ✅ Default Purity JSONs (filtered by project)
        $purityDefaultData = Product::join('product_variants', 'product_variants.product_id', '=', 'products.id')
            ->where('products.Project', $request->project_id)
            ->where('product_variants.qty', '>', 0)
            ->whereNotNull('product_variants.Purity')
            ->where('product_variants.Purity', '!=', '')
    ...
    public function getProcategorywiseProduct(Request $request)
    {
        ...
        $user_id = Auth::user()->id;
        $selectedprocategory = (array) $request->input('selectedprocategory', []);
        $products = (array) $request->input('productArray', []);
        $purity = (array) $request->input('purity', []);

        // ✅ Get selected procategory
        $getProcategory = Product::whereIn('Procatgory', $selectedprocategory)->pluck('Procatgory')->toArray();

        // ✅ Base query for filter options (only depends on Procategory + Project)
        $optionBaseQuery = Product::join('product_variants', 'product_variants.product_id', '=', 'products.id')
            ->where('products.Project', $request->project_id)
            ->whereIn('products.Procatgory', $getProcategory)
            ->where('product_variants.qty', '>', 0);

        // Collect all available Purities for selected Procategories
        $allPurityIds = $optionBaseQuery->clone()
            ->whereNotNull('product_variants.Purity')
            ->where('product_variants.Purity', '!=', '')
            ->pluck('product_variants.Purity')
        ...
        if (!empty($request->project_id)) {
            $procategorywiseproduct->where('products.Project', $request->project_id);
        }

        if (!empty(array_filter($products))) {
            $procategorywiseproduct->whereIn('products.Item', $products);
        }

        if (!empty(array_filter($purity))) {
            $procategorywiseproduct->whereIn('product_variants.Purity', $purity);
        }

        $procategorywiseproduct = $procategorywiseproduct->where('product_variants.qty', '>', 0)->orderBy('products.DesignNo', 'ASC');
        ...
        // ✅ Purity JSONs (only valid for selected Procategories)
        $purityData = Product::join('product_variants', 'product_variants.product_id', '=', 'products.id')
            ->where('products.Project', $request->project_id)
            ->whereIn('product_variants.Purity', $allPurityIds)
            ->where('product_variants.qty', '>', 0)
        ...
        if ($products) {
            $purityData = $purityData->whereIn('products.Item', $products)->get();
        } else {
            $purityData = $purityData->get();
        }
        ...
        if ($products) {
            $purityDefaultData = $purityDefaultData->whereIn('products.Item', $products)->get();
        } else {
            $purityDefaultData = $purityDefaultData->get();
        }
    ...
    public function getPuritywiseProduct(Request $request)
    {
        ...
        $user_id = Auth::user()->id;
        $selectedpurity = (array) $request->input('selectedpurity', []);
        $productsInput = $request->input('productArray', []);
        $procategoriesInput = $request->input('procategory', []);

        // If string, convert to array
        $products = is_array($productsInput)
            ? $productsInput
            : (strlen($productsInput) ? explode(',', $productsInput) : []);

        $procategories = is_array($procategoriesInput)
            ? $procategoriesInput
            : (strlen($procategoriesInput) ? explode(',', $procategoriesInput) : []);

        // ✅ Collect selected Purity values
        $getPurity = ProductVariant::whereIn('Purity', $selectedpurity)
            ->pluck('Purity')
            ->toArray();

        // ✅ Base query for filter options (depends on Purity + Project)
        $optionBaseQuery = Product::join('product_variants', 'product_variants.product_id', '=', 'products.id')
            ->where('products.Project', $request->project_id)
            ->whereIn('product_variants.Purity', $getPurity)
            ->where('product_variants.qty', '>', 0);

        // Collect all available Procategories for selected Purities
        $allProcategoryIds = $optionBaseQuery->clone()
            ->whereNotNull('products.Procatgory')
            ->where('products.Procatgory', '!=', '')
            ->pluck('products.Procatgory')
        ...
        if (!empty($request->project_id)) {
            $puritywiseproduct->where('products.Project', $request->project_id);
        }

        if (!empty(array_filter($products))) {
            $puritywiseproduct->whereIn('products.Item', $products);
        }

        if (!empty(array_filter($procategories))) {
            $puritywiseproduct->whereIn('products.Procatgory', $procategories);
        }

        $puritywiseproduct = $puritywiseproduct->where('product_variants.qty', '>', 0)->orderBy('products.DesignNo', 'ASC');
        ...
        // ✅ Procategory JSONs (only valid for selected Purities)
        $procategoryData = Product::join('product_variants', 'product_variants.product_id', '=', 'products.id')
            ->where('products.Project', $request->project_id)
            ->whereIn('products.Procatgory', $allProcategoryIds)
            ->where('product_variants.qty', '>', 0)
            ...
            ->groupBy('products.Procatgory');
        if ($products) {
            $procategoryData = $procategoryData->whereIn('products.Item', $products)->get();
        } else {
            $procategoryData = $procategoryData->get();
        }
        $procategoryjson = $procategoryData->toJson();

        // ✅ Default Procategory JSONs (filtered by project)
        $procategoryDefaultData = Product::join('product_variants', 'product_variants.product_id', '=', 'products.id')
            ->where('products.Project', $request->project_id)
            ->where('product_variants.qty', '>', 0)
            ->whereNotNull('products.Procatgory')
        ...
        if ($products) {
            $procategoryDefaultData = $procategoryDefaultData->whereIn('products.Item', $products)->get();
        } else {
            $procategoryDefaultData = $procategoryDefaultData->get();
        }
